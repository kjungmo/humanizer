...
**가드 교체.** 변경률 30%/50% 가드는 `general`과 `dev-blog`에서만 유효하다. 이 둘만 어투·구조·분량을 원문에서 가져온다. 네이버 블로그와 인스타 3종은 어투를 옮기고 분량을 채널 규격에 맞추는 순간 사실상 재작성이라 변경률이 의미를 잃는다. 그 자리를 **사실 대장**이 대신한다.
